...
| 2 | 🏷️ **タグの格上げ**を承認する（候補を正式タグにする） | 管理者 |
...
起動・排他・復旧・入力制限の修正と適用手順は [docs/REVIEW-FIXES.md](docs/REVIEW-FIXES.md) を参照してください。
**旧エージェントを停止して `0015_agent_safety.sql` を適用してから新コードを起動してください。**
本番には `CRON_SECRET` の設定も必要です。コードだけの差し替えでは動きません。

### 画面遷移・性能改善版の適用

[変更内容と適用手順](docs/PERFORMANCE-FIXES.md)を参照。旧フォルダへの上書きでは削除済みloading.jsが残るため、新しいフォルダへ展開する。DBの0016を適用してから再ビルドする（0015が未適用なら先に適用）。
