...
import { estimateCallCost, positiveNumber } from './budget.mjs'
import { createContext } from './runtime.mjs'
import { organizerPrompt as SYSTEM } from './prompts.mjs'

export async function runOrganizer(options = {}) {
const { db, ai, cleanupDb } = createContext('organizer', options)
...
const BUDGET_USD = Number(env.ORGANIZER_BUDGET_USD || 1.0)   // 1周あたりのコスト上限



positiveNumber(BUDGET_USD, 'ORGANIZER_BUDGET_USD')
const inputRate = positiveNumber(Number(env.ORGANIZER_INPUT_USD_PER_MILLION || 10), 'input rate')
const outputRate = positiveNumber(Number(env.ORGANIZER_OUTPUT_USD_PER_MILLION || 50), 'output rate')
let spent = 0
...
async function callLLM(userContent) {
  // Conservative reservation. It is NOT a price guarantee for dynamically routed models.
  const estimated = estimateCallCost(SYSTEM, userContent, inputRate, outputRate)
  if (unknownCost || !Number.isFinite(spent) || spent + estimated > BUDGET_USD) return null   // ★ コスト上限。呼び出し側は wait 扱いにする
  ...
  const reportedCost = data.usage?.cost_usd
  const cost = typeof reportedCost === 'number' && Number.isFinite(reportedCost) && reportedCost >= 0 ? reportedCost : null
...
try {
  if (!DRY_RUN) {
    const { data, error } = await db.rpc('start_organizer', { p_model: MODEL, p_day: options.cronDay ?? null })
  ...
  const readFailure = reads.find(r => r.error)
  if (readFailure) throw readFailure.error
  const [{ data: tags }, { data: allUserTags }, { data: allUsers }, { data: activeQuests }, { data: recentInvites }] = reads
  ...
  const { data: endedLives, error: endedError } = await fetchAll(() => db.from('lives').select('id, topic_tag_id, ended_at').eq('status', 'ended').not('topic_tag_id', 'is', null).order('id'))
  ...
  if (runId) {
    const { error } = await db.from('agent_runs').update({ status: 'succeeded', cost_usd: unknownCost ? null : spent, request_ids: requestIds, finished_at: new Date() }).eq('id', runId)
    if (error) throw error
  }
  ...
  console.log(unknownCost ? `   費用 不明（既知分 $${spent.toFixed(6)}）。追加呼び出しを停止しました` : `   費用 $${spent.toFixed(6)}（停止目安 $${BUDGET_USD.toFixed(2)}）\n`)
  return { summary, cost: unknownCost ? null : spent }

} catch (e) {
  console.error('\n🛑 失敗:', e.message)
  if (runId) await cleanupDb.from('agent_runs')
    .update({ status: 'failed', error: String(e.message), cost_usd: unknownCost ? null : spent, request_ids: requestIds, finished_at: new Date() }).eq('id', runId)
  throw e
}
