...
import { mirrorPrompt as SYSTEM } from './prompts.mjs'

export async function runMirror(options = {}) {
const { db, ai, cleanupDb } = createContext('mirror', options)
...
const IMAGE_PATHS = options.frames ?? []
const STDIN_META = options.meta ?? {}
...
try {
  ...
  if (!DRY_RUN) {
    const { data, error } = await db.rpc('start_mirror', { p_user: USER_ID, p_model: MODEL })
    if (error) { if (error.code === 'P0429') error.status = 429; throw error }
    runId = data
  }
  ...
  // At most two requests at once, and await all in-flight calls before unlocking on failure.
  const perImage = []
  for (let i = 0; i < IMAGE_PATHS.length; i += 2) {
    const batch = await Promise.allSettled(IMAGE_PATHS.slice(i, i + 2).map((p, j) => analyzeOne(p, i + j)))
    const failed = batch.find(r => r.status === 'rejected')
    if (failed) throw failed.reason
    perImage.push(...batch.map(r => r.value))
  }
  const requestIds = []
