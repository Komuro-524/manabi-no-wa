...
//   middleware はセッション更新を行う。ここは表示用であり、認証済みと判断しない。
...
const loadProfile = cache(async uid => {
  const db = await supabaseServer()
  const { data, error } = await db.from('users').select('id, display_name, department, role').eq('id', uid).maybeSingle()
  if (error) throw new Error('プロフィールを取得できませんでした。再度お試しください')
  return data
})
