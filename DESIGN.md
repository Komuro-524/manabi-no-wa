...
| セキュリティ | 短命トークン（10分）・1部屋限定・聞き手は `canPublish: false`・終了で部屋ごと削除・DTLS-SRTP／WSS／TURN-TLS・鍵はサーバーだけ・**自社テナントで録音なし 音声保存なし**・Webhook 署名検証・人数と費用の上限 |
| ネットワーク | UDP → TCP(7881) → **TURN over TLS(443)** の順に降りる。SSL インスペクションの対象外設定・許可リストは自社の2ドメインだけ・音声のみ Opus で約32kbps・環境別の検証表と接続テスト画面 |
| 両立しないもの | E2EE とサーバー側 STT。今回は「社内から出さない」で守る |

> 既知の壁には全部 越え方を当ててある。**本当に通るかは 情シスと検証表を埋めて確かめる**（§10 の方針どおり）。
