...
3. タグを決める → **三重の門**（§5）を通す
4. 話者を名簿と照合する
   - 照合は **空白を落とした正規化名**（`星野陸` = `星野 陸`）
   - **名簿に無い話者の社員アカウントを作らない**。未知のまま人に返す
   - 未知の話者がいたライブは `done` ではなく **`ingest_status = 'needs_review'`** で止まる
5. `user_tags` を更新する（候補タグでも作る）
6. 昇格の条件を満たしたタグを `proposed` にする（**自分で `official` にはしない**）
...
6. タグ候補は最初から `user_tags` に `source='self'`, `visibility='private'` で入る
...
| 3 | 🙋 **未知の話者の名寄せ** | 管理者 | 名簿に無い名前が出たライブは `needs_review` で止まる。**AIは社員を作らない** |
...
| 15 | **エージェントは社員アカウントを作らない**。名簿に無い話者は `needs_review` で人に返す | 表記ゆれ（`星野陸` / `星野 陸`）で同一人物が増殖し、知見も興味も分散して全部の判断が狂う |
