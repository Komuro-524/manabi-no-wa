...
| **タグ辞書**（管理者画面の名前は「タグ帳」） | `tags` テーブル。**候補・格上げ候補・正式・弾いた語・禁止を1枚で持つ**。全員に見えるのは正式タグだけ（§5.4） |
| **格上げ候補** | 条件を満たしたので Aが「正式にしませんか」と管理者に出したタグ（`proposed`）。**正式にできるのは管理者だけ** |
| **知見カード** | ライブから切り出された知識のかけら。タグと1対1。話した人が紐づく |
| **ライブのタネ（quest）** | 場づくりエージェントが抱えている未完了の仕事。日をまたいで1つのゴール（ライブを1本立てる）を追う。段階は 育ち待ち → 話し手に相談中 → 日程を決め中 → ライブ予約済み → 開催済み。旧称「企て」、DB は `quests` |
| **話した言葉** | ライブ中にマイクで話して文字になった発言（`transcript_segments`）。チャットのコメントとは別に持つ（§11） |
| **提供形態** | 🅰️ 標準プラン（開発元が国内クラウドで運用・契約日から使える）と 🅱️ 専用環境プラン（導入企業のテナントに構築）。コードは同じ（§11.3） |
...
| **読む** | トランスクリプト・チャット | タグ・ライブのタネ・予定・過去の結末 | 画面の静止画 |
| **書く** | 知見カード・タグ | ライブのタネ・ライブ・通知 | タグ候補（本人にだけ） |
| **人に喋る** | **できない** | できる | できない |
...
1. 文字起こし（`transcript_segments`）とチャットを読む
   ...
   - 入口は3つ。デモの文字起こしファイル（`open-live.mjs`）／デモのマイク入力（`/api/lives/[id]/segments` がログイン中の本人の `user_id` で書く・§11）／将来の音声通話の文字起こし係（トークンに書いた `users.id` で書く・§11）。**どの入口でも名前で人を決めない**
   - 文字起こしが0行でもチャットがあれば取り込む（コメントだけのライブ）
...
**トリガー**: 1日1回 自分で起きる ＋ 進行中のライブのタネがあれば毎回見に行く

- 本番: `vercel.json` と `/api/cron/organizer` に1日1回の定期実行を実装。`CRON_SECRET` の設定と本番デプロイ後に有効になる。
- デモ: `scripts/organizer-loop.mjs` が間隔を縮めて自分で回す（既定5分・最大6周で止まる）。管理者画面の「いま1周回す」からも起こせる（二重起動は弾く）

**やること（順番）**

1. 進行中のライブのタネを全部見て、それぞれの次の一手を決める
2. 新しく立てる価値のあるタグを探す
3. **「立てるか」を毎回判断する**（§3.3）
...
| 2 | 🏷️ **タグ格上げの承認** | 管理者 | `proposed` → `official`。タグ帳の単語帳で1枚ずつ仕分ける（採用＝正式／却下＝弾いた語）。本人が「タグにしてほしい」と申請したタグは上に出る |
...
- **★ 9/22 追加: 本人の申請。** 少数しか知らない知識は「延べ3回 または 2人以上」に届きにくい。そこで、話した本人が自分に付いた育ちかけのタグに「タグにしてほしい」と申請できる（`tag_requests`・関数 `request_tag()`・0020）。
  申請は**管理者の画面で上に出すだけ**で、正式にするのは今まで通り管理者の承認だけ。申請できるのは自分の `user_tags` にある `candidate`/`proposed` のタグに限る
- **B は状態を見ない**（§8 ルール9・14）。正式化は「全社の語彙になるか」、ライブのタネは「人と人をつなぐか」で別のレイヤー
...
データも輪になっている。**ライブ → 知見カード → 人のタグ → ライブのタネ → またライブ。**
...
| `tag_requests` | 育ちかけのタグへの「タグにしてほしい」申請（0020）。書き込みは関数 `request_tag()` だけ。読めるのは本人と管理者 | 本人 | 🟡 |
| `agent_cron_days` | 場づくりの日次起動を同じ日に二重に走らせないための記録（0015） | B | 🟡 |
...
-- ★ タグごとに 進行中のライブのタネは1つまで（毎日同じ部屋が立つのを防ぐ）
...
-- ★ update ポリシーは作らない（quest_id を書き換えて他人のライブのタネに割り込める）
...
-- ライブのタネ・判断ログ・実行記録：ポリシーを作らない
--   ＝ ブラウザからは一切見えない。エージェント（service_role）だけが触る

-- 文字起こし（transcript_segments）：select は参加者だけ（0008）。★ insert ポリシーは作らない
--   ＝ ブラウザから他人の発言を作れない。デモのマイク入力は /api/lives/[id]/segments が
--     本人（getUser）・配信中・このライブの話し手 を確かめてから、本人の user_id で service_role が書く（§11）
```
...
| 1.5 | **update ポリシーは「どの列を」縛れない。** 列を守りたいものは update ポリシーを作らず `security definer` 関数にする（`user_tags` `invitations` も同じ） | 強さを自分で盛る／他人のライブのタネに割り込む |
...
| 10 | ライブのタネの停止条件（14日／3人／コスト上限）は **コードで持つ。LLMに任せない** | ループが止まらず課金され続ける |
...
| 14 | ライブのタネは **`user_tags` の重なりだけで判断する**。正式か候補かで絞らない（ただし人が禁止した `banned` と、門1で弾いた `rejected` は対象外） | ルール9が崩れる／弾いた語・禁止語で場が立つ |
| 15 | **話者を名前で照合しない。** 文字起こしは `user_id` 付きの行で受け取り、エージェントは社員アカウントを作らない | 表記ゆれ（`星野陸` / `星野 陸`）で同一人物が増殖する。1行書き足すだけで **他人の名義でタグが作れる** |

> **9 の補足**: タグの正式化は「**全社の語彙になるか**」、ライブのタネは「**人と人をつなぐか**」。
...
| 長期運用（数千人 × 10年） | 直せる所は直した。残りはリスクとして §12 に書いた |
| 本番デプロイ・定期実行 | **今回はしない。** デモは手元（`npm run build` → `npm run start`、`localhost:3030`）。定期実行の代わりに `organizer-loop.mjs` で自走を見せる |
| 提供形態（🅰️標準／🅱️専用環境） | 決めたうえで実装はしない（§11.3）。コードは同じで 置き場所と鍵の持ち主が変わるだけ |
| 音声通話（声を他の参加者に届ける） | デモは **ブラウザの音声入力で文字だけを共有**（§11）。声を届ける構成は §11 と `docs/VOICE-DESIGN.md` に決めてある |
...
詳細は **`docs/VOICE-DESIGN.md`**。ここは要点だけ。

### 11.1 今回のデモ（実装済み）

| 項目 | 内容 |
|---|---|
| 何ができる | 話し手がライブ中に「🎙️ マイクで話す」→ ブラウザの音声認識で文字にし、**確定した一文だけ** を `transcript_segments` に入れる |
| 書き込み口 | `POST /api/lives/[id]/segments`。本人（`getUser`）・配信中・**このライブの話し手** を確かめ、`user_id` は本人で固定して service_role で書く。ブラウザ向けの insert ポリシーは作らない |
| 限界 | 声は他の参加者に届かない。声はブラウザ提供元のサーバーで文字になる（開始前に同意を取る）。Chrome／Edge のみ |
| 変えていないもの | recorder・知見カード・RLS。**音声の入口が変わっても 文字から先は同じ** |

### 11.2 将来の音声通話（構成だけ決めてある）

| 層 | 決めていること |
|---|---|
| 部品 | SFU（LiveKit を自前で建てる）＋ まなびのわのトークン発行 API ＋ 話し手ごとの STT エージェント → `transcript_segments` |
| 誰の声か | トークンの `identity = users.id`。サーバーが参加者・配信中を確かめてから発行（名前では決めない） |
| セキュリティ | 短命トークン（10分）・1部屋限定・聞き手は `canPublish: false`・終了で部屋ごと削除・DTLS-SRTP／WSS／TURN-TLS・鍵はサーバーだけ・**§11.3 で決めた場所から出さない（録音なし 音声保存なし）**・Webhook 署名検証・人数と費用の上限 |
| ネットワーク | UDP → TCP(7881) → **TURN over TLS(443)** の順に降りる。SSL インスペクションの対象外設定・許可リストはまなびのわの2ドメインだけ・音声のみ Opus で約32kbps・環境別の検証表と接続テスト画面 |
| 両立しないもの | E2EE とサーバー側 STT。今回は「社内から出さない」で守る |

> 既知の壁には全部 越え方を当ててある。**本当に通るかは 導入企業の情シスと検証表を埋めて確かめる**（§10 の方針どおり）。

### 11.3 サーバーを誰が持つか（提供形態）

「サーバーを建てる」の主語は **2通り** ある。声だけでなく まなびのわ全体を同じ場所に置く。

| | 🅰️ 標準プラン（SaaS） | 🅱️ 専用環境プラン |
|---|---|---|
| 建てて運用する | **開発元（私たち）** | **導入企業** のテナント（構築は私たちが手伝う） |
| 使い始め | ✅ 契約した日から | ❌ 構築に数週間〜 |
| 声と文字 | 私たちの国内クラウドで処理・録音なし・声は保存しない | 導入企業の環境から出ない |
| 向いている相手 | 中小〜中堅・まず試したい会社 | 金融・官公庁・大企業 |

- どちらでも **導入企業の情シスの通信許可**（2ドメイン・TLS 復号の対象外）と **セキュリティ審査** は残る。443番だけで通る作りと 審査の回答集で軽くする
- 外部の通話サービス（他社の SaaS）は どちらのプランでも使わない（第三者のサーバーを声が通るため）
- 詳細は `docs/VOICE-DESIGN.md` §2.0

## 12. 長期運用のリスク（数千人 × 10年）

「数千人の会社が10年使う」と仮定して、壊れるところを洗い出した。**今すぐ直せたものは直し**、設計の見直しが要るものは **リスクとして残す**。

### 12.1 直したもの（9/21 夜）

| 壊れ方 | 直し方 | 場所 |
|---|---|---|
| **1001人目から名前が「?」になる**。8画面が全社員を毎回読んでいた。Supabase は1回1000行が既定の上限で、超えた分は**エラーなしで欠ける** | 画面に出てくる人だけを読む（200件ずつ `in`） | `lib/users-by-id.js` |
| **エージェントの入力が黙って欠ける**。場づくりが読むタグ・人のタグ・社員、タグ付けが読む辞書、**2時間のライブの文字起こし**が1000行で切れていた | ページを送って全部読む。天井（5万行）を超えたら**止めて知らせる**（黙って切らない） | `lib/fetch-all.mjs`（`organizer.mjs` `recorder.mjs`） |
| ライブ一覧が全ライブ・全参加記録を読んでいた | 配信中・予定・最近の過去12件と その参加者だけ | `livehub` |
| メンバー画面が全員を1枚に出していた | 50人ずつのページ分け。数えるのもそのページの人だけ | `admin/people` |
| 知識地図・タグ帳・ライブのタネが全件を読んでいた | ページ送りで全部読む／件数に上限 | `map` `admin/tags` `admin/progress` `admin/quests` |

### 12.2 残るリスク（設計の見直しが要る）

🔴 早く効く　🟡 数年で効く　⚪ 10年で効く

| | リスク | 何が起きるか | 見直しの方向 |
|---|---|---|---|
| 🔴 | **画面の読み直しが重い** | 参加者全員が6秒おきに画面を作り直す。200人のライブで毎秒30回以上 | Supabase Realtime で変わった分だけ配る |
| 🔴 | **エージェントの起動が画面のサーバー頼み** | ✅ 9/22 に一部解消（§13）: 子プロセスをやめ、サーバー関数から直接 `await` する形にした。DBの排他・リース・日次Cronで `pending` も拾う。**残り**: 1回の処理が300秒を超える長いライブ | 長い処理はジョブキューへ |
| 🔴 | **集計を画面側でしている** | ✅ 9/22 に解消（§14・0016）: 知識地図・タグ帳・月の費用・カレンダーは DB 側の関数で集計する | — |
| 🔴 | **場づくりが一部のタグしか見ない** | 1周の費用上限は同じでタグだけ増える。毎回同じ順で見て上限で止まり **後ろのタグに順番が来ない** | 前回見た順を覚えて回す／部署ごとに分けて回す |
| 🔴 | **タグ辞書を丸ごと LLM に渡している** | 辞書が数千語になるとプロンプトが膨らみ **費用と精度が落ちる** | 近い語だけを選んで渡す（検索してから渡す） |
| 🟡 | **退職者を消せない** | 発言は参加者表への外部キーで守っている。**守りが固いぶん消すと連鎖で詰まる**。削除の依頼に応えにくい | 消さずに**匿名化**（名前を外し知見は残す）。保存期間を決める |
| 🟡 | **組織改編に弱い** | 部署はただの文字列。合併・改称のたびに地図とメンバー画面が崩れる | 部署を別の表にし、名前の変更履歴を持つ |
| 🟡 | **タグ辞書が散らかる** | 似たタグを寄せる操作が無い（§10）。製品名は名前が変わる | 管理者の「まとめる」と 過去のカードの付け替え |
| 🟡 | **AIモデルの廃止・入れ替え** | プロンプトは今のモデルに合わせてある。入れ替えで出力が変わる | 採点用のテスト問題を持ち、入れ替え前に同じ結果が出るか確かめる |
| 🟡 | **鍵の期限切れ** | OrcaRouter の鍵3本に有効期限を付けた。**切れた日にエージェントが全部止まる** | 期限の前に知らせる。入れ替えの手順を書く |
| 🟡 | **自動テストが少ない** | ✅ 9/22 に着手: `tests/`（RLS・入力制限・排他・画面の読み直し）を `npm run test` で回せる。**残り**: 画面の操作（タグ帳・申請など）のテスト | 画面の操作を Playwright で自動テストにする |
| 🟡 | **月の費用の集計** | ✅ 9/22 に解消: DB 側の関数 `admin_month_cost`（0016）で合計する | — |
| ⚪ | **知見が古くなる** | 知見カードに賞味期限が無い（興味タグにはある）。昔の手順が「詳しい人」として出続ける | カードに見直し時期を付け、話した本人に確かめる |
| ⚪ | **「詳しい人」が固定される** | タグは溜まる一方。昔詳しかった人が打診され続け、新しく詳しくなった人が見えない | タグの強さを時間で減らす |
| ⚪ | **土台の技術の世代交代** | Next.js・Supabase・ブラウザの音声認識は10年で何世代も変わる。型の無い JavaScript は引き継ぎにくい | 段階的に TypeScript へ。依存の更新を年中行事にする |

> 設計の芯（名前で人を決めない／AIは候補まで・正式は人／停止条件はコードで持つ）は変えなくてよい。壊れるのは **量と時間の運用の側** で、作り直しではなく足していけば直る。



## 13. コードレビュー後の実行・復旧設計（2026-09-22）

- HTTPリクエストから子プロセスを起動しない。`lib/agents/server.js` をサーバー専用の入口にし `lib/agents/*.mjs` の関数を `await` する。CLIは同じ関数を呼ぶ薄い入口とする。
- 秘密情報はサーバー専用の入口またはCLIから渡す。共用モジュールはimport時に環境変数を読まず、プロセスを終了せず、実行ごとの状態を共有しない。`service_role` の環境変数を読むサーバーファイルの先頭に `import 'server-only'` を置く。
- Functionsの上限を300秒、アプリの外部通信期限を240秒とする。終了処理には別の短い通信期限を設ける。長い録音はジョブキューが必要。
- BはDB関数と部分ユニーク制約で1実行だけにする。CronはUTC日付も記録し同日の再配信をスキップする。強制終了したBのロックを時刻だけで外さない。実行終了と副作用を確認して管理者が解除する。
- Aはライブごとのトークンと10分のリースで排他する。カード・タグ・強度・語られた記録・アンコール・完了記録は1トランザクションにする。失敗は全体を戻す。期限切れの古い実行は新しいトークンで上書きできない。
- Aの失敗は最大3回まで再試行し以後 `needs_review`。未処理または期限切れは日次Cronが1件ずつ拾う。旧実装の `running` は部分書き込みの可能性があるため移行時に `needs_review` にする。
- Cは2〜12枚、画像1枚512KB、リクエスト全体4MB、1920×1080以内の静止画に制限する。MIME・Base64・実際のデコードを検査する。画像を保存しない。LLMの並列数は2まで。DBで本人ごとに同時1回・5分間隔・24時間に10回までに制限する。
- Bの費用判定は推定入力量・最大出力2048トークン・設定単価・25%の余裕を使う。未知の費用は0円にせず追加呼び出しを停止する。ルーターの価格や課金境界はアプリだけでは保証できない。厳密な請求上限は外部のキー予算設定で確認する。
...
ライブのタネは認証・管理者権限を検証するJSON APIで局所更新する。DBの画面用集計は0016に追加。knowledge_map_statsとcalendar_livesはsecurity invokerで本人のRLSを適用し、管理者集計はservice_roleだけに実行を許可する。知識地図は最大60タグ、タネは100件ずつ、ライブ履歴はコメント100件・文字起こし400行ずつ表示する（§15）。

変更内容・適用手順・再現と検証の条件はdocs/PERFORMANCE-FIXES.mdを参照。

## 15. 画面の追加と見直し（2026-09-22）

9/21 夜の画面確認（チーム2人で実際に操作）で出た要望を反映した。**データの守り（§7・§8）は変えていない。** 増えた書き込みは 0020 の申請だけで、それも関数経由・本人の分だけ。

| 画面 | 変えたこと | 守りとの関係 |
|---|---|---|
| 右上の検索 → `/search` | すべての画面で同じ検索窓。知見カード・ライブ（予定も）・タグ・メンバーを種類ごとの段で出す。メンバーは公開中の正式タグを添える | 本人のセッションで読む。RLS で見えるものしか出ない |
| 他の人のプロフィール `/people/[id]` | アイコン・名前・検索結果から開ける。自分のプロフィールと同じ形。「この人に聞けそう／話せそう」（自分の興味 × 相手の知見） | **本人が公開した正式タグだけ**。管理者が見ても非公開・育ちかけは出さない |
| 知見カード | ページ専用の検索窓。タグを複数選べる（OR） | — |
| 知識地図 | ドラッグで移動・ホイールで拡大縮小・タグ検索で飛ぶ・分野で絞る | 集計は `knowledge_map_stats`（0016） |
| ライブ中の画面 | **文字起こしとチャットを別の枠にした**。文字起こしは常に表示し、いつでも上にさかのぼれる（最新を見ているときだけ自動で追う） | 読めるのは参加者だけ（今まで通り） |
| プロフィール | 育ちかけのタグに「タグにしてほしい」ボタン | `request_tag()`（0020）。自分の育ちかけタグだけ |
| 管理者: タグ帳（旧タグ辞書） | 開くと申請つき・格上げ候補の単語帳が出る。クリックでめくって採用、引っぱると穴の横が破れて却下 | 更新は管理者本人のセッション（RLS「管理者だけタグを裁ける」） |
| 管理者: ライブのタネ | 列をプランター（土）に。植物が進み具合で育つ（種→芽→葉→つぼみ→花）。同じ段階の中は需要で大きさを変える。見回り中は畑番が歩き、いま決めたことを1行で出す | 読むだけ |
| 管理者: タネの足あと | 選ぶのをブラウザの中だけにして軽くした | 読むだけ |
| 管理者: メンバー | 話し手の負担を名前ごとの棒グラフで | 読むだけ |
| AIからの打診 | 見た目をモック（`repos/mock/mock_new`）に合わせた | 興味を持つ人は名前を出さず人数と部署だけ（今まで通り） |
| 全体 | 七宝模様をごく薄く敷いた・ロゴを透過版に | — |

**デモ用データ**: `0021_ui_demo_seed.sql`（架空のデータだけ）。展示で触ってもらったあとは `supabase/demo/reset-demo.sql` でデモの行だけを最初の状態に戻す（本番の運用データには触らない）。

---

*最終更新: 2026-09-22 / 変更したらこのファイルに書き戻すこと*
