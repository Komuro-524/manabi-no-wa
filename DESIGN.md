...
| 知識地図・タグ辞書・ライブのタネが全件を読んでいた | ページ送りで全部読む／件数に上限 | `map` `admin/tags` `admin/progress` `admin/quests` |
...
| 🔴 | **エージェントの起動が画面のサーバー頼み** | 子プロセスで起こしている。再起動で消える。途中で止まったもの（`running`）は人に返せるが **始まる前（`pending`）は拾えない**。サーバーレス（Vercel）では動かない | ジョブキューに載せて再試行。`pending` のまま古いものを見張る |
| 🔴 | **集計を画面側でしている** | 知識地図・タグ辞書は全行を読んでから数える。今は上限で欠けないが **行が増えるほど遅い** | DB 側で集計する（ビューや関数。マイグレーションが要る） |
...
| 🟡 | **自動テストが無い** | 証拠スクリプトは動作確認の記録で、回帰テストではない。直すたびに別の所が壊れうる | RLS と権限を自動テストにする（いちばん先にやる） |
| 🟡 | **月の費用の集計** | ダッシュボードは今月の実行記録を全部読んで足している | DB 側で合計する |
