...
| 管理者の「まとめる」「禁止する」操作 | 今の管理者の操作は **承認だけ**（`approve-tag.mjs`）。禁止は DB で `banned` に書き換える。似たタグ（例: `インデックス` と `インデックス最適化`）が別々に格上げ候補に上がってきても、**片方に寄せる操作は無い**。ただし辞書には最初から寄せ先の列 `alias_of` があり、エージェントもそれを見て新しい発言を寄せる。**足りないのは過去のカード・人のタグ・語られた記録を付け替える処理だけ** |
