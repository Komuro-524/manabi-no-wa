...
const TABS = [['proposed', '格上げ候補'], ['candidate', '候補'], ['official', '正式'], ['rejected', '弾いた語'], ['banned', '禁止']]
...
export default async function AdminTags({ searchParams }) {
  ...
  const rows = (all ?? []).filter(t => t.status === tab)
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {TABS.map(([k, name]) => (
            <Link key={k} href={`/admin/tags?tab=${k}`} className={'tab' + (tab === k ? ' tabon' : '')}>
              {name}<span className="num" style={{ opacity: .7 }}>{(all ?? []).filter(t => t.status === k).length}</span>
            </Link>
          ))}
        </div>
        {tab === 'proposed' && <div className="note">格上げ候補 ＝ 直近30日に「延べ3回（3つのライブ）」または「2人以上」が語った言葉。正式にすると、その言葉の知見カードと、公開にしている人のタグが全員に見えるようになります</div>}
        <div className="card sh" style={{ padding: 12 }}>
          {rows.length === 0 ? <div className="empty">ありません</div> : (
            <table>
              <thead><tr><th>タグ</th><th>種類</th><th>語られた（30日）</th><th>語られた（全期間）</th><th>{tab === 'official' ? '正式になった日' : 'できた日'}</th><th></th></tr></thead>
              <tbody>
                {rows.map(t => {
                  const s = stat.get(t.id)
                  return (
                    <tr key={t.id}>
                      <td style={{ fontWeight: 700 }}>{t.name}{t.rejected_reason && <div className="sub" style={{ fontWeight: 400 }}>{t.rejected_reason}</div>}</td>
                      <td className="sub">{t.kind}</td>
                      <td className="num">{s ? `${s.recentLives.size}回・${s.recentUsers.size}人` : '—'}</td>
                      <td className="num sub">{s ? `${s.lives.size}回・${s.users.size}人` : '—'}</td>
                      <td className="sub">{fmtWhen(tab === 'official' ? (t.promoted_at ?? t.created_at) : t.created_at)}</td>
